...
public class AdminView extends VerticalLayout {
    ...
    private final ShiftService shiftService; // Service for Staff KPIs
    ...
    private Grid<StaffStatsDTO> staffGrid; // Grid for Employee Stats
    ...
    public AdminView(UserService userService, DishService dishService,
        ...
        H1 title = new H1(getTranslation("admin.title")); // "Admin Panel"
        ...
        Tab staffStatsTab = new Tab("Staff KPIs"); // New Tab
        ...
        Tabs tabs = new Tabs(dashboardTab, staffStatsTab, usersTab, menuTab, reservationsTab, ordersTab, dataTab);
        tabs.addClassName("admin-tabs");

        // --- Content Containers ---
        Div dashboardContent = createDashboardTab();
        Div staffStatsContent = createStaffStatsTab(); // New Content
        ...
        staffStatsContent.setVisible(false);
        ...
        tabsToPages.put(staffStatsTab, staffStatsContent);
        ...
        tabs.addSelectedChangeListener(event -> {
            ...
            if (selectedPage == staffStatsContent) refreshStaffStats();
        ...
        refreshDashboard(); // Load initial dashboard data
    }
    ...
    // ================== 2. STAFF KPIs TAB (NEW) ==================
    private Div createStaffStatsTab() {
        H2 title = new H2("Employee Performance");
        title.addClassName("admin-section-title");

        staffGrid = new Grid<>(StaffStatsDTO.class, false);
        staffGrid.addClassName("admin-grid");

        staffGrid.addColumn(StaffStatsDTO::getEmployeeName).setHeader("Employee");
        staffGrid.addColumn(StaffStatsDTO::getTotalShifts).setHeader("Total Shifts");
        staffGrid.addColumn(stats -> String.format("%.1f", stats.getTotalHours())).setHeader("Total Hours Worked");

        return new Div(new VerticalLayout(title, staffGrid));
    }

    private void refreshStaffStats() {
        staffGrid.setItems(shiftService.getStaffStatistics());
    }
    ...
    private Div createUserManagementTab() {
        ...
        addUserButton.addClickListener(click -> {
            ...
            userForm.setUser(new User()); // Open form for new user
        });
    ...
    private UserForm createUserForm() {
        ...
        form.addListener(UserForm.SaveEvent.class, event -> {
            ...
            updateUserGrid();
            form.setUser(null);
            Notification.show(getTranslation("msg.saved", "User saved"), 3000, Notification.Position.TOP_CENTER)
        ...
        form.addListener(UserForm.DeleteEvent.class, event -> {
            try {
                if (event.getUser().getEmail().equals("admin@kinto.com")) {
                    Notification.show("Cannot delete main admin.", 3000, Notification.Position.TOP_CENTER)
    ...
    private DishForm createDishForm() {
        ...
        form.addListener(DishForm.SaveEvent.class, event -> {
            ...
            updateMenuGrid();
            form.setDish(null);
            Notification.show("Dish saved.", 3000, Notification.Position.TOP_CENTER)
                    .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
        });
    ...
    private Div createReservationsTab() {
        ...
        reservationGrid.addColumn(r -> r.getUser() != null ? r.getUser().getFullName() : "N/A")
                .setHeader(getTranslation("res.customer"));
        ...
        reservationGrid.addColumn(r -> r.getReservationDate() != null ?
                        r.getReservationDate().format(DateTimeFormatter.ofPattern("dd.MM.yyyy")) : "N/A")
                .setHeader(getTranslation("res.date"));
        reservationGrid.addColumn(r -> r.getReservationTime() != null ?
                        r.getReservationTime().format(DateTimeFormatter.ofPattern("HH:mm")) : "N/A")
    ...
    private ReservationForm createReservationForm() {
        ...
        form.addListener(ReservationForm.SaveEvent.class, event -> {
            try {
                ...
                Notification.show("Reservation saved.", 3000, Notification.Position.TOP_CENTER)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
    ...
    private void refreshActiveOrders() {
        ...
        // Fetch active orders using correct method name
    ...
    private Div createOrderCard(Order order) {
        ...
        // Buttons for Admin Order Management
    ...
    private Div createDataManagementTab() {
        ...
        HorizontalLayout menuActions = new HorizontalLayout(
                createExportButton(getTranslation("btn.export_json"), "menu.json", true, false),
                createExportButton(getTranslation("btn.export_xml"), "menu.xml", false, false),
                createImportUpload(getTranslation("btn.import_json"), true, false),
                createImportUpload(getTranslation("btn.import_xml"), false, false)
        );
        ...
        HorizontalLayout userActions = new HorizontalLayout(
                createExportButton(getTranslation("btn.export_json"), "users.json", true, true),
                createExportButton(getTranslation("btn.export_xml"), "users.xml", false, true),
                createImportUpload(getTranslation("btn.import_json"), true, true),
                createImportUpload(getTranslation("btn.import_xml"), false, true)
        );
    ...
    private Anchor createExportButton(String text, String filename, boolean isJson, boolean isUsers) {
        ...
        // Determine endpoint
        String url = isUsers ?
                (isJson ? "/api/export/users/json" : "/api/export/users/xml") :
                (isJson ? "/api/export/menu/json" : "/api/export/menu/xml");
    ...
    private Upload createImportUpload(String text, boolean isJson, boolean isUsers) {
        ...
        upload.addSucceededListener(event -> {
            try {
                ...
                Notification.show("Import successful!", 3000, Notification.Position.TOP_CENTER)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                updateUserGrid();
                updateMenuGrid();
            } catch (Exception e) {
                Notification.show("Import failed: " + e.getMessage(), 5000, Notification.Position.TOP_CENTER)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
            }
        });
        return upload;
