...
public class AuthView extends VerticalLayout implements BeforeEnterObserver {
    ...
    private Div createLoginForm() {
        ...
        H1 title = new H1(getTranslation("auth.login.title", "Welcome Back"));
        title.addClassName("auth-title");

        H2 subtitle = new H2(getTranslation("auth.login.subtitle", "Sign in to book a table"));
        subtitle.addClassName("auth-subtitle");

        EmailField email = new EmailField(getTranslation("auth.email", "Email"));
        email.setPlaceholder("your@email.com");
        email.setWidthFull();
        email.addClassName("auth-input");

        PasswordField password = new PasswordField(getTranslation("auth.password", "Password"));
        password.setPlaceholder("••••••••");
        password.setWidthFull();
        password.addClassName("auth-input");

        Button loginBtn = new Button(getTranslation("btn.signin", "Sign In"), e -> login(email.getValue(), password.getValue()));
        loginBtn.addClassName("auth-btn");
        loginBtn.addThemeVariants(ButtonVariant.LUMO_PRIMARY);

        Button toRegister = new Button(getTranslation("auth.link.register", "Create an account"), e -> showForm(false));
    ...
    private Div createRegisterForm() {
        ...
        H1 title = new H1(getTranslation("auth.register.title", "Join Kinto"));
        title.addClassName("auth-title");

        H2 subtitle = new H2(getTranslation("auth.register.subtitle", "Create your account"));
        subtitle.addClassName("auth-subtitle");
        ...
        TextField fullName = new TextField(getTranslation("auth.fullname", "Full Name"));
        fullName.setPlaceholder("John Doe");
        fullName.setWidthFull();
        fullName.addClassName("auth-input");

        TextField phone = new TextField(getTranslation("auth.phone", "Phone"));
        ...
        PasswordField confirmPassword = new PasswordField(getTranslation("auth.confirm_password", "Confirm Password"));
        confirmPassword.setPlaceholder("••••••••");
        confirmPassword.setWidthFull();
        confirmPassword.addClassName("auth-input");

        Button registerBtn = new Button(getTranslation("btn.create_account", "Create Account"), e -> register(
        ...
        Button backBtn = new Button(getTranslation("auth.link.login", "Already have an account? Sign in"), e -> showForm(true));
