...
public class MainLayout extends AppLayout {
    ...
    private void createHeader() {
        ...
        H1 logoTitle = new H1(getTranslation("app.title", "Kinto"));
        logoTitle.addClassName("nav-title");
        logoTitle.getStyle().set("margin", "0 20px 0 0"); // Отступ справа
        logoTitle.getStyle().set("font-size", "1.2em"); // Чуть меньше, чтобы не мешал
        logoTitle.getStyle().set("flex-shrink", "0");

        // 2. Навигация (Центр, с прокруткой)
        Tabs navTabs = createNavigation();
        navTabs.setWidthFull();
        navTabs.getStyle().set("margin", "0");
        navTabs.getStyle().set("overflow", "hidden"); // Скрываем скроллбар, но оставляем функционал (обычно Vaadin сам хэндлит)

        // Обертка для табов, чтобы они занимали все место, но не вытесняли кнопки
        Div tabsContainer = new Div(navTabs);
        tabsContainer.setWidthFull();
        tabsContainer.getStyle().set("flex-grow", "1");
        tabsContainer.getStyle().set("min-width", "0"); // Важно для Flexbox, чтобы позволить сжиматься

        // 3. Кнопки управления (Справа)
        Select<Locale> languageSelect = createLanguageSwitcher();
        Button notificationButton = createNotificationButton();
        Button loginLogoutButton = createLoginLogoutButton();
        ...
        controlsLayout.getStyle().set("margin-left", "20px"); // Отступ слева от меню
        controlsLayout.setFlexGrow(0); // Фиксированный размер
        controlsLayout.setFlexShrink(0); // Не сжимать кнопки!

        // --- ГЛАВНЫЙ КОНТЕЙНЕР ---
        HorizontalLayout header = new HorizontalLayout(logoTitle, tabsContainer, controlsLayout);

        header.setDefaultVerticalComponentAlignment(FlexComponent.Alignment.CENTER);
        header.setWidth("100%");
        header.addClassName("nav-container");
        header.setPadding(true);
        header.setSpacing(false); // Мы сами управляем отступами
    ...
    private void showNotificationsDialog(User user) {
        ...
        // Метод обновления списка, вызывается при открытии и после удаления
        refreshNotificationsList(list, user);

        Button close = new Button("Close", e -> {
            dialog.close();
            UI.getCurrent().getPage().reload(); // Перезагружаем страницу, чтобы обновить счетчик на колокольчике
        });
        dialog.getFooter().add(close);
    ...
    // --- ИСПРАВЛЕНИЕ: Метод для заполнения и обновления списка уведомлений ---
    private void refreshNotificationsList(VerticalLayout list, User user) {
        list.removeAll();
        var notifications = notificationService.getUserNotifications(user);
        ...
        if (notifications.isEmpty()) {
            ...
            for (var n : notifications) {
                // Если непрочитано, помечаем как прочитанное (опционально)
                if (!n.isRead()) {
                    notificationService.markAsRead(n.getId());
                }
                ...
                // Текст сообщения
                Span msg = new Span(n.getMessage());
                msg.getStyle().set("font-size", "0.9rem");
                if (!n.isRead()) {
                    msg.getStyle().set("font-weight", "bold");
                }

                // Дата
                Span date = new Span(n.getTimestamp().format(DateTimeFormatter.ofPattern("HH:mm dd/MM")));
                date.getStyle().set("font-size", "0.75rem").set("color", "gray");

                // Контейнер для текста (чтобы дата была под сообщением или рядом)
                VerticalLayout textLayout = new VerticalLayout(msg, date);
                textLayout.setSpacing(false);
                textLayout.setPadding(false);

                // Кнопка УДАЛИТЬ (Крестик)
                Button deleteBtn = new Button(VaadinIcon.CLOSE_SMALL.create(), e -> {
                    notificationService.deleteNotification(n.getId());
                    refreshNotificationsList(list, user); // Обновляем этот же список
                });
                deleteBtn.addThemeVariants(ButtonVariant.LUMO_TERTIARY, ButtonVariant.LUMO_SMALL, ButtonVariant.LUMO_ERROR);
                deleteBtn.setTooltipText("Delete notification");

                // Строка списка
                HorizontalLayout row = new HorizontalLayout(textLayout, deleteBtn);
                row.setWidthFull();
                row.setAlignItems(FlexComponent.Alignment.CENTER);
                row.setJustifyContentMode(FlexComponent.JustifyContentMode.BETWEEN);

                // Заставляем текст занимать все доступное место, тесня кнопку вправо
                row.expand(textLayout);
